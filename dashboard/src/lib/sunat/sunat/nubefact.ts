...
// Inline structured logger — replaces yaya-business shared/logger import.
// Kept intentionally minimal so this module stays self-contained.
const logger = {
  info: (data: Record<string, unknown>, msg: string) =>
    console.log(JSON.stringify({ level: 'info', component: 'sunat/nubefact', msg, ...data })),
  error: (data: Record<string, unknown>, msg: string) =>
    console.error(JSON.stringify({ level: 'error', component: 'sunat/nubefact', msg, ...data })),
};
