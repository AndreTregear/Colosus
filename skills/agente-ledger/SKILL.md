# agente-ledger — Simplified Daily Sales & Cash Ledger
...
- `agente-expenses` — Cross-reference for P&L: ledger revenue + expenses = profit
- `agente-fiados` — When payment_method='fiado', cross-link to fiado tab
- `agente-analytics` — Feed daily totals into analytics for trend/comparison features
...
Payment method detection:
  ...
  "lo anotó / se lo llevó fiado" → fiado (→ link to agente-fiados)
...
- **agente-expenses**: Ledger provides revenue side. Expenses provides cost side. Together = P&L. When owner asks "¿cuánto gané?", combine both.
- **agente-fiados**: When a sale is logged as "fiado", auto-create or update the customer's tab in agente-fiados. "La vecina Carmen se llevó fiado S/12" → ledger entry (sale, fiado) + fiado tab update.
- **agente-analytics**: Daily summaries feed into analytics for trend analysis, day-of-week patterns, seasonality detection.
