# agente-billing — Subscription & Billing Management

## Description
Manages Agente Platform's own subscription billing for its business clients via Lago. Checks subscription status, generates invoices, handles plan changes (upgrades/downgrades), sends payment reminders, and tracks usage. This is the B2B billing layer — how Agente Platform gets paid by the businesses that use it. Not to be confused with agente-payments, which handles the business's customer-facing payment validation.
...
- A payment for the Agente Platform subscription is overdue
- Usage metrics need to be checked against plan limits
- A free trial is about to expire
- A new client has been onboarded and needs billing setup (triggered by agente-onboarding)
- The Agente team needs billing reports or revenue metrics
...
```
[Día 1 de atraso]
Agente: "Hola 👋 Tu factura de Agente Platform por S/249 + IGV venció ayer.
...
[Día 8 — escalate to Agente billing team]
```

### Trial Expiry Warning
```
[3 días antes de que expire el trial]
Agente: "Hola 👋 Tu prueba gratuita de Agente Platform termina en 3 días.
...
Agente: "Cancelación procesada ✅
         ...
         Gracias por haber sido parte de Agente. Si cambias de opinión,
         aquí estaremos 🙏"
```
...
- **Lago API unreachable:** Queue billing operations and retry. Never tell a client their billing action failed without trying at least 3 times. If still failing, escalate to Agente engineering team.
...
- **Disputed charges:** Log the dispute, pause collection on the disputed amount, escalate to Agente billing team. Never auto-resolve disputes.
...
- **Refund requests:** Process refunds for the current billing period only. For older periods, escalate to Agente billing team.
