# agente-notifications — Bulk Notifications & Outreach Campaigns

## Description
Manages bulk and segmented customer notifications via WhatsApp for small businesses. Handles restock alerts, payment reminders, promotional campaigns, and operational notices at scale. Unlike agente-followup (which handles 1:1 proactive messages triggered by individual customer events), agente-notifications orchestrates batch outreach to customer segments — think "notify all 47 customers who asked about Nike Air Max" or "send a payment reminder to everyone with pending orders." Respects opt-outs, enforces rate limits, and formats everything for WhatsApp (no markdown tables, ever).
...
- **Rate Limiting** — Enforce max 1 promotional message per customer per week, no overlap with agente-followup messages
...
- **Payment reminders are not promos.** They're transactional. These don't count toward the weekly promo limit but still respect the daily frequency cap from agente-followup.
...
- **Coordinate with agente-followup.** Check if the customer already received a message today before adding to a campaign batch.
...
3. Build audience list:
   ...
   d. Filter out customers contacted today (coordinate with agente-followup)
4. Compose message:
...
Agente: "✅ Campaña completada — Air Max 90 restock
         ...
         Ya los estoy atendiendo con agente-sales."
```
...
- **Customer responds to campaign:** Route to agente-sales or appropriate skill. The campaign message becomes the start of a conversation.
- **Overlap with agente-followup:** Before including a customer in a campaign batch, check if agente-followup already messaged them today. If so, skip and include in next batch.
