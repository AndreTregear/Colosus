# agente-memberships — Recurring Subscription & Membership Management
...
- **Payment Collection** — Integrate with agente-payments for Yape/Plin receipt verification. Track who's paid, who hasn't, and send targeted reminders
...
- **Never store payment credentials.** Only process screenshots/receipts via agente-payments
...
Agente: [verifica pago via agente-payments, crea membresía en postgres-mcp, crea contacto en crm-mcp]
...
Agente: [espera comprobante, verifica via agente-payments]
...
- **Payment mismatch:** If the Yape amount doesn't match the plan price, handle via agente-payments partial payment flow. Don't block enrollment for minor rounding differences.
