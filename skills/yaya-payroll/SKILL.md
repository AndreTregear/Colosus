# yaya-payroll — Staff Payment & Shift Tracking
...
Supports multiple payment types: fixed monthly salary, daily rate (jornal), hourly rate, and commission-based (via yaya-commissions). Integrates with yaya-expenses to automatically log payroll as a business cost. Sends WhatsApp pay summaries to staff so everyone agrees on the numbers.
...
- **Peluquerías/salones** — 50-80K. Commission-based via yaya-commissions, but some have fixed-rate assistants too.
...
- **Pay types** — Fixed monthly salary, daily rate (jornal), hourly rate, commission-based (links to yaya-commissions)
...
- **Auto-log from appointments** — For service businesses using yaya-appointments, auto-detect working hours from completed appointments
...
- **Commission integration** — For commission-based staff, pull totals from yaya-commissions instead of calculating from hours
...
- **Auto-expense** — Every payroll payment auto-creates an expense in yaya-expenses (categoría: planilla)
...
|-------------|-----------|---------------|
| yaya-commissions | Commission payout | Replace hourly calc with commission total for commission-based staff |
| yaya-expenses | Payroll payment | Auto-log each payment as planilla expense |
| yaya-cash | Cash payment | Track cash payroll payments in caja |
| yaya-appointments | Completed services | Auto-detect working hours for service-based staff |
| yaya-fiados | — | No direct link |
...
- `yaya-expenses` — Auto-log payroll payments as business expenses (categoría: planilla)
- `yaya-commissions` — Pull commission totals for commission-based staff
- `yaya-cash` — Cash payroll payments tracked in caja
- `yaya-appointments` — Auto-detect working hours from completed services (optional)
- `yaya-notifications` — Send pay summaries to staff via WhatsApp
...
CREATE TABLE IF NOT EXISTS business.payroll_payments (
    ...
    commission_total NUMERIC(12,2) DEFAULT 0,  -- from yaya-commissions (if applicable)
    payment_method TEXT DEFAULT 'efectivo' CHECK (payment_method IN (
        'efectivo', 'yape', 'plin', 'transferencia'
    )),
    expense_id UUID,                           -- link to yaya-expenses record
...
Agente: [consulta yaya-commissions]
Agente: "📊 María — Semana 14-20 marzo (comisiones)
         ...
         (Detalle de comisiones viene de yaya-commissions)"
```
...
- `PAYROLL_AUTO_EXPENSE` — Auto-create expense in yaya-expenses when payment recorded (default: true)
