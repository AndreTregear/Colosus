# agente-production — Artisan Network & Production Order Management
...
This skill replaces that chaos with structured production tracking — without requiring the artisans themselves to use any system. The owner (Rosa, the taller owner) is the sole user. She tells Agente what she needs made, who's making it, and when it's due. Agente tracks the rest.
...
- **Link to sales order** — If order came from agente-quotes or agente-sales, auto-link for traceability
...
- **Material cost tracking** — Links to agente-expenses: "La lana alpaca me costó S/45/kg" → feeds into per-unit production cost
- **Integration: agente-inventory** — Stock levels for raw materials. Deductions on material issue, additions on returns

### Core: Production Costing
- **Per-unit cost calculation** — Automatic: materials + labor = production cost per unit
  - Materials: BOM × material cost (from agente-expenses/agente-inventory)
  - Labor: artisan payment per unit (from agente-commissions/agente-expenses)
...
- **Integration: agente-expenses** — Material costs, artisan payments flow into expense tracking for P&L
...
- **Integration: agente-commissions** — Artisan payments tracked as per-piece commissions
- **Integration: agente-expenses** — Artisan payments recorded as planilla/mano_de_obra expenses
...
- `agente-expenses` — Material costs, artisan payments as expenses
- `agente-commissions` — Per-piece artisan payment tracking
...
CREATE TABLE IF NOT EXISTS production.orders (
    ...
    sales_order_id UUID,                  -- link to agente-sales/agente-quotes if applicable
...
- **The owner is the only system user.** Artisans don't have smartphones, don't use apps, and shouldn't be required to. The owner manages everything through WhatsApp with Agente.
