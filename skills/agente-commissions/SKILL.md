# agente-commissions — Staff Commission, Tip & Payout Tracking for Service Businesses
...
The skill bridges service attribution (agente-appointments tracks who performed which service) and business cost tracking (agente-expenses records payroll as a cost) by managing what happens in between: who did what, how much they earned, what deductions apply, and what gets paid out on Friday.
...
- **Auto-attribution from appointments** — When agente-appointments records a completed service, auto-attribute to the assigned stylist
...
- **Payout confirmation** — "Ya le pagué a Ana" → marks payout as completed, creates expense in agente-expenses
...
- **Client retention rate** — Percentage of clients who return to the same stylist (requires agente-appointments data)
...
CREATE TABLE IF NOT EXISTS business.commission_services (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    staff_id UUID NOT NULL REFERENCES business.commission_staff(id) ON DELETE CASCADE,
    appointment_id UUID,                   -- link to agente-appointments if auto-attributed
...
- **Create expense entry** — When payout is confirmed, auto-create expense in agente-expenses (category: planilla/comisiones)
...
Agente: "✅ Comisiones configuradas:
...
Agente: "✅ Servicios de *Ana* registrados — Viernes 21/03
...
Agente: [genera resumen semanal]
Agente: "💰 *Pagos de la semana* — 15 al 21 de marzo
...
Agente: [marca pagos como completados]
      [envía resumen individual a cada empleada via whatsapp-mcp]
      [crea gasto en agente-expenses: S/1,309.50, categoría: planilla/comisiones]
Agente: "✅ Pagos registrados y resúmenes enviados:
       📲 Ana — enviado ✅
       📲 Paola — enviado ✅
       📲 Lucía — enviado ✅
...
Agente: "Te muestro el detalle completo de Ana esta semana:
...
Dueña: "Sí, mándale"
Agente: [envía detalle a Ana via whatsapp-mcp]
Agente: "📲 Detalle enviado a Ana ✅"
```

### Performance Question
```
Dueña: "¿Quién es mi mejor estilista?"
Agente: "📊 *Ranking de marzo* (hasta hoy):
...
Agente: "✅ Tus servicios registrados — Viernes 21/03
...
Agente: "⏱️ *Utilización de Lucía — Marzo*
