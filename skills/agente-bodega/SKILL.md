# agente-bodega — Corner Store Operations for Peru's 500K+ Bodegas
...
- **Integration: agente-ledger** — Each quick sale creates a ledger entry for accounting
- **Integration: agente-cash** — Cash sales auto-update cash drawer balance
- **Integration: agente-inventory** — Stock decremented automatically after sale confirmation
...
- **Integration: agente-cash** — Full handoff to agente-cash for reconciliation, denomination tracking, shortage investigation

### 3. Fiado Integration (Ventas al Crédito)
- **Seamless handoff** — "Se lo llevó fiado la señora María" → delegates to agente-fiados for tab creation
- **Quick balance check** — "¿Cuánto me deben?" → queries agente-fiados for total outstanding
- **Credit warning** — Before creating fiado: "Ojo: la Sra. María ya te debe S/45" → informed decision
- **Integration: agente-fiados** — Full fiado lifecycle managed by agente-fiados; agente-bodega provides the entry point
...
- **Integration: agente-inventory** — Stock levels from ERPNext; alerts via WhatsApp
...
- **Integration: agente-suppliers** — PO creation, delivery tracking, price history
...
- **Integration: agente-suppliers** — Auto-suggest increased order quantities
...
- **Integration: agente-crm** — Full customer profiles stored in CRM, fiado data from agente-fiados
...
- **Integration: agente-voice** — TTS via Kokoro, sent as WhatsApp voice note
- **Integration: agente-cash** — Cash reconciliation data
- **Integration: agente-analytics** — Sales and trend data
...
CREATE TABLE IF NOT EXISTS bodega.quick_sales (
    ...
    fiado_tab_id UUID,                 -- reference to agente-fiados tab if credit sale
...
- End-of-day reconciliation via agente-cash
- Recharge and service payment cash tracked in cash drawer

### With agente-fiados
- "Se lo llevó fiado" seamlessly creates/appends fiado tab via agente-fiados
- Quick balance checks routed to agente-fiados
- Fiado collections recorded as cash-in via agente-cash
...
- Distributor order templates create POs via agente-suppliers
- Delivery tracking and price history maintained by agente-suppliers
