# agente-forecast — Demand Forecasting, Smart Reorder & Purchase Planning

## Description
WhatsApp-native demand forecasting and purchase planning for Peru's 1M+ commerce businesses (bodegas, ferreterías, tiendas), 143K+ restaurants, and 80K+ service businesses. Answers the universal MYPE question: **"¿Cuánto compro mañana?"** Analyzes historical sales patterns from agente-ledger/agente-restaurant/agente-analytics to predict demand by day-of-week, detect seasonality, generate smart shopping lists for mayorista trips, set reorder alerts, track commodity price trends, and estimate purchase budgets. Starts simple (last-week-same-day heuristic after just 7 days of data), grows smarter with time (moving averages, seasonal decomposition after 4+ weeks). Zero configuration — just works once the owner has been logging sales.
...
- Stock is running low and reorder is needed (proactive alert from agente-inventory integration)
...
- **Integrates with agente-suppliers** — Can auto-generate PO to preferred supplier when reorder triggered
...
- **Cash flow awareness** — Integrates with agente-ledger: "Hoy tienes S/[X] en caja. Tus compras de mañana cuestan ~S/[Y]. Te quedan S/[X-Y] de margen"
...
CREATE TABLE product_demand (
    ...
    current_stock DECIMAL(10,2),           -- Last known stock (from agente-inventory)
...
- `agente-ledger` — Daily sales data (revenue, transaction counts, product-level if available)
- `agente-restaurant` — Dish-level sales data, recipe ingredients for recipe explosion
- `agente-inventory` — Current stock levels, stock movements
- `agente-suppliers` — Supplier info, lead times, last purchase prices
- `agente-expenses` — Purchase costs, price history for budget estimation
- `agente-analytics` — Historical sales trends, period comparisons
...
Waste/overstock:
  ...
  "¿Cuánto estoy botando?" → waste_report() (integrates with agente-restaurant waste tracking)
...
¿Quieres que arme el pedido? (integración con agente-suppliers)
```
...
- **With agente-ledger:** Reads daily sales data. If no ledger data yet, prompts: "Para darte mejores predicciones, cuéntame tus ventas del día con agente-ledger"
- **With agente-restaurant:** Reads dish sales for recipe explosion (dish → ingredients → purchase list)
- **With agente-inventory:** Reads current stock for reorder calculations. If inventory not tracked, estimates from purchase frequency
- **With agente-suppliers:** Pulls lead times and preferred suppliers for reorder alerts. Can auto-generate PO
- **With agente-expenses:** Reads purchase costs for budget estimation and price trend tracking
...
daily_demand_snapshot:
  ...
  action: |
    1. Pull today's sales from agente-ledger/agente-analytics
...
reorder_check:
  ...
  condition: "Only if agente-inventory integration is active"
  action: |
...
## Error Handling
- **No sales data yet:** "Aún no tengo datos de tus ventas. ¿Quieres que te ayude a empezar a registrar con agente-ledger?"
...
- Shopping lists may be shared with suppliers via agente-suppliers integration (owner must approve first).
