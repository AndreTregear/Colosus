# agente-salon — Beauty Salon & Spa Operations Management
...
- **agente-commissions** — Staff commission calculation and payout (handles the money side)
- **agente-appointments** — Base appointment scheduling (handles time slots)
- **agente-loyalty** — Points and rewards program (handles retention)
- **agente-inventory** — Retail product stock (handles SKUs)
- **agente-expenses** — Product purchase costs, salon overhead (handles P&L)
- **agente-cash** — Daily register reconciliation (handles cuadre)
- **crm-mcp** — Client contact records (handles contact data)
- **whatsapp-mcp** — Outbound messaging (handles reminders, confirmations)

What agente-salon adds that NO other skill covers:
...
- **Boleta on gift card sale** — Integrates with agente-tax: gift card sale generates boleta at purchase time
...
- **Suggestion integration** — Feed into agente-appointments for availability display, into marketing for promo timing
...
While agente-inventory handles product SKUs, salons need a service-specific menu with durations, pricing tiers, and stylist assignments.
...
Agente: "✅ Fórmula guardada para Sra. Rodríguez:
🎨 Igora 6.1 + 7.1 (50/50)
💧 Oxidante 20vol
...
Agente: "María Gutiérrez — última coloración 15-feb-2026:
...
Agente: "Balayage necesita ~2.5-3 horas. Veamos disponibilidad:
...
Agente: "🎁 Gift Card GC-8K4M-2R7P — Válida ✅
Saldo: S/300.00
...
Agente: "😤 Plantón registrado — Lucía Vargas, 3:00pm
...
Agente: "🪑 Utilización semanal (lun-sáb):
...
Agente: "⚠️ Alerta de alergia registrada:
Cliente: [nombre]
...
- Flag for immediate restock via agente-inventory
