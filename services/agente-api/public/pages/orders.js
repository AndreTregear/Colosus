...
function renderOrders(orders) {
  ...
  tbody.innerHTML = orders.map(o => `
    <tr>
      <td><code>${esc(o.id?.slice(0, 12))}</code></td>
...
window._viewOrder = async (id) => {
  try {
    ...
    YayaModal.open(`Pedido ${id.slice(0, 12)}`, `
      <div class="mb-16">
        <strong>Cliente:</strong> ${esc(order.customer_name || 'Sin cliente')}<br>
        <strong>Estado:</strong> <span class="badge ${statusClasses[order.status]}">${statusLabels[order.status]}</span><br>
        <strong>Fecha:</strong> ${formatDateTime(order.created_at)}<br>
        ...
        <tbody>
          ${items.map(i => `
            <tr>
              ...
              <td class="text-right">${formatMoney(i.unit_price)}</td>
...
async function showNewOrderModal() {
  ...
  try { customers = (await apiGet('/customers')).data || []; } catch {}
  try { products = (await apiGet('/products')).data || []; } catch {}
